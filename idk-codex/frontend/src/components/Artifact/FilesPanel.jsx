/**
 * FilesPanel — side drawer showing all files in the current session.
 *
 * Files are pulled from IndexedDB (fileStore) and also re-fetched from the
 * server sandbox. Clicking a file opens the ArtifactPreview modal.
 */
...
function detectLang(path) {
  ...
  const map = {
    ...
    ts: 'typescript', tsx: 'tsx', json: 'json', py: 'python', md: 'markdown',
    svg: 'svg', txt: 'text'
  };
  return map[ext] || 'text';
}
...
export default function FilesPanel({ sessionId, open, onClose, onOpenFile }) {
  ...
  async function loadFiles() {
    setLoading(true);
    try {
      // From IndexedDB (local)
      const local = await listFiles(sessionId);
      setFiles(local);

      // From server sandbox (in case files exist there but not locally)
      try {
  ...
  async function handleDelete(file) {
    ...
    await deleteFile(file.sessionId, file.path);
    loadFiles();
  }

  async function handleDownload(file) {
    await downloadFile(file.sessionId, file.path);
  }

  // Merge local + server files (local takes priority)
    ...
    <div className="fixed inset-0 z-40 flex justify-end" onClick={onClose}>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto">
          {loading && files.length === 0 ? (
            ...
            <div className="text-center py-12 px-4">
              <Folder size={48} className="mx-auto text-gray-700 mb-3" />
              <p className="text-gray-500 mb-1">No files yet</p>
              <p className="text-xs text-gray-600">
                Files the agent creates will appear here. They're saved locally in your browser, so they survive restarts and work offline.
              </p>
            </div>
          ) : (
              ...
              {allFiles.map((file, idx) => {
                ...
                return (
                  <div
                    ...
                    className="flex items-center gap-3 px-4 py-3 hover:bg-gray-900 cursor-pointer group"
                  >
                    <div
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <span className="text-sm text-gray-100 truncate">{filename}</span>
                        {file.source === 'server' && (
        ...
        <div className="px-4 py-2 border-t border-gray-800 bg-gray-900 text-xs text-gray-500">
          Files are stored locally in your browser (IndexedDB).
        </div>
      </div>
    </div>
  );

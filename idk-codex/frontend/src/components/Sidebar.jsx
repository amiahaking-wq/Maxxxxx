 ...
 *
 * Slides in from the left. Lists all past conversations from Supabase,
 * lets the user switch, rename, or delete them. Clicking a conversation
 * loads it in the chat area without a page reload.
 */

import { useState, useEffect, useCallback } from 'react';
import { useNavigate } from 'react-router-dom';
import {
  X, Plus, MessageSquare, Trash2, Edit2, Check, Search, Clock
} from 'lucide-react';
...
export default function Sidebar({ open, onClose, currentSessionId, onSwitchSession, onNewChat }) {
  ...
  const handleDelete = async (e, convId) => {
    ...
    try {
      await fetch(`${API_BASE}/api/conversations/${convId}?userId=web_user`, { method: 'DELETE' });
  ...
  const handleRenameSave = async (e, conv) => {
    ...
    try {
      await fetch(`${API_BASE}/api/conversations/${conv.id}`, {
        ...
        body: JSON.stringify({ userId: 'web_user', title: editingTitle.trim() })
      });
      setConversations(prev => prev.map(c => c.id === conv.id ? { ...c, title: editingTitle.trim() } : c));
    } catch (e) { /* ignore */ }
    setEditingId(null);
  };
  ...
  return (
    <div className="fixed inset-0 z-40 flex" onClick={onClose}>
      ...
      <div
        className="relative bg-gray-950 border-r border-gray-800 w-[85%] max-w-sm h-full flex flex-col"
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-gray-800">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-gray-100">Chats</h2>
        ...
        <div className="p-3 border-b border-gray-800">
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 bg-gray-900 border border-gray-800 rounded-lg text-sm text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            />
          </div>
        </div>
            ...
            <div className="divide-y divide-gray-900">
              {filtered.map(conv => {
                ...
                return (
                  <div
                    key={conv.id}
                    onClick={() => !isEditing && handleSwitch(conv)}
                    className={`flex items-center gap-3 px-4 py-3 cursor-pointer transition-colors group ${
                      isActive ? 'bg-gray-900 border-l-2 border-blue-500' : 'hover:bg-gray-900 border-l-2 border-transparent'
                    }`}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      {isEditing ? (
                        <input
                          ...
                          onClick={(e) => e.stopPropagation()}
                          onKeyDown={(e) => e.key === 'Enter' && handleRenameSave(e, conv)}
                          className="w-full px-2 py-1 bg-gray-800 border border-blue-500 rounded text-sm text-white focus:outline-none"
                          autoFocus
                        />
                      ) : (
                        <>
                          <div className={`text-sm truncate ${isActive ? 'text-white font-medium' : 'text-gray-300'}`}>
                            {conv.title || 'Untitled'}
                          </div>
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {isEditing ? (
                        <button
                          onClick={(e) => handleRenameSave(e, conv)}
        ...
        <div className="px-4 py-2 border-t border-gray-800 bg-gray-900 text-xs text-gray-500">
          Chats are stored in Supabase and persist forever.
        </div>
      </div>
    </div>
  );
